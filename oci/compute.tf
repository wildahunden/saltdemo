...
resource "oci_core_instance" "saltdemo-ws1" {
  count                 = "1"
  # LessonsLearned: the availability domain is unique to the tenancy.  Can't use someone elses list.
  availability_domain   = "aNUQ:US-ASHBURN-AD-1"
  compartment_id        = "${var.compartment_ocid}"
  # LessonsLearned: if you leave out the display_name, the display in the console uses a long string that's hard to identify.  Use this always.
  ...
  # LessonsLearned:  Use ssh-keygen to generate this pair (not openssl)
  ...
  metadata {
    ...
    # LessonsLearned: changes to cloud-init script cause the instance to be destroyed and re-created as part of the terraform apply process.  Need to be careful what is put in here.  Don't want production servers to be re-provisined inadvertently.
